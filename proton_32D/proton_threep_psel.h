...
#endif
    inline void load_prop_psrc_threep(const std::string &job_tag, const int traj,
                                      const int dtmax, const int tsrc, const PointSelection &psel,
                                      const Geometry &geo)
    {
        TIMER_VERBOSE("load_prop_psrc_threep");
        ...
        for (long nsrc = 0; nsrc < n_points; ++nsrc)
        {
            ...
            for (long nsnk = 0; nsnk < n_points; ++nsnk)
            {
                ...
                const double coef_sphere = real(lat_data_complex_get_const(coef_table,
                                                                           make_array(dist1))[0]);
                psel_dt_num_list[dt] += coef_sphere;
            }
        }
...
#pragma omp parallel for
            for (long nsnk = 0; nsnk < n_points; ++nsnk)
            {
                ...
                {
                    ppblock.block_pp[igram] *= coef_sphere;
                }
    ...
    inline void contract_proton_threep_unshifted_psel_acc_x(
    ...
    inline void contract_proton_threep_unshifted_psel(std::vector<SelectedField<Complex>> &proton_threep,
                                                      const std::string &job_tag, const int traj,
    ...
    {
        TIMER_VERBOSE("contract_proton_threep_unshifted_psel");
...
#pragma omp parallel for
            for (long idx = 0; idx < fsel.n_elems; ++idx)
            {
                ...
                if (tsrc <= tsnk)
                {
                    contract_proton_threep_unshifted_psel_acc_x(vv, coef, wm_x_snk, wm_x_src, block1);
                }
                else
                {
                    const Complex coef1 = -1.0 * coef;
                    contract_proton_threep_unshifted_psel_acc_x(vv, coef1, wm_x_snk, wm_x_src, block1);
                }
            }
        }
    ...
    inline void
    contract_proton_three_point_acc_psel(
    ...
    {
        TIMER_VERBOSE("contract_proton_three_point_acc_psel");
        const Geometry &geo = fsel.f_rank.geo();
        ...
        contract_proton_threep_unshifted_psel(sfs, job_tag, traj, prop_x_src, xg_src, xg_src_psel_idx,
    ...
    inline std::string get_proton_three_point_func_psel_path(const std::string &job_tag,
                                                             const int traj)
    {
        return ssprintf("analysis/proton_threep_psel_coef/%s/results=%d", job_tag.c_str(),
                        traj);
    }

    inline void compute_proton_three_point_func_psel_type(const std::string &job_tag, const int traj,
                                                          const std::vector<int> &gram_convert_list)
    {
        ...
        const std::string path = get_proton_three_point_func_psel_path(job_tag, traj);
        ...
        const std::string path_coef_table = ssprintf("coef-list/twop/coef-twop-80slice.lat");
        LatData coef_twop;
        ...
        TIMER_VERBOSE("compute_proton_three_point_func_psel_type");
        ...
        contract_proton_pp_block(block, psel_num_list, psel_dt_num_list, job_tag, traj, geo, psel, dtmax, dtmin, coef_twop);
        displayln_info(fname + ssprintf("test_num: %.1f", psel_dt_num_list[0]));
        long iter = 0;
        for (int tsrc = 0; tsrc < total_site[3]; ++tsrc)
        {
            load_prop_psrc_threep(job_tag, traj, dtmax, tsrc, psel, geo);
            for (long nsrc = 0; nsrc < n_points; ++nsrc)
            {
                const long xg_src_psel_idx = nsrc;
                const Coordinate &xg_src = psel[xg_src_psel_idx];
                ...
                for (int tsep = dtmin; tsep <= dtmax; ++tsep)
                {
                    ...
                    contract_proton_three_point_acc_psel(threep_data[dt], job_tag, traj, prop_x_src, xg_src,
    ...
    inline void compute_proton_three_point_func_psel(const std::string &job_tag,
                                                     const int traj)
    {
        check_sigterm();
        Timer::autodisplay();
        TIMER_VERBOSE("compute_proton_three_point_func_psel");
        const std::string path = get_proton_three_point_func_psel_path(job_tag, traj);
        ...
        check_time_limit();
        if (not obtain_lock(
                ssprintf("lock-proton-threep-psel-coef-%s-%d", job_tag.c_str(), traj)))
        {
            return;
        }
        setup(job_tag, traj);
        qmkdir_info("analysis/proton_threep_psel_coef");
        qmkdir_info(ssprintf("analysis/proton_threep_psel_coef/%s", job_tag.c_str()));
        qmkdir_info(path);
        ...
        compute_proton_three_point_func_psel_type(job_tag, traj, gram_convert_list);
        release_lock();
    }
}
