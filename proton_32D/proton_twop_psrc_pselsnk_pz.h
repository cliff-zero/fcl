...
#pragma omp for
            for (long idx = 0; idx < (long)psel.size(); ++idx)
            {
                ...
                c_ts[tsep] += proton_twop_block(wm, wm, wm, 0) * cp;
                c_ts[tsep] -= proton_twop_block(wm, wm, wm, 1) * cp;
            }
    ...
    inline std::string get_proton_two_point_psrc_pselsnk_path(const std::string &job_tag,
                                                              const int traj)
    {
        return ssprintf("analysis/proton_twop_pselsnk_pz/%s/results=%d", job_tag.c_str(),
                        traj);
    }
                                                      ...
                                                      const int traj)
    {
        ...
        for (long n = 0; n < n_points; n++)
        {
            ...
            LatData ld = contract_proton_two_point_psrc_pselsnk_function(pprop, geo, tslice, psel, xg_src);
    ...
    inline void compute_proton_two_point_psrc_pselsnk_func(const std::string &job_tag, const int traj)
    {
        ...
        check_time_limit();
        if (not obtain_lock(
                ssprintf("lock-two-point-pselsnk-func-%s-%d", job_tag.c_str(), traj)))
        {
            return;
        }
        setup(job_tag, traj);
        qmkdir_info("analysis/proton_twop_pselsnk_pz");
        qmkdir_info(ssprintf("analysis/proton_twop_pselsnk_pz/%s", job_tag.c_str()));
        qmkdir_info(path);
