...
#pragma omp parallel for
    ...
    inline void contract_proton_pp_y_block(const std::vector<std::vector<std::vector<ProtonPPBlock>>> &block,
                                           ...
                                           const int dtmin, const int tsep, const LatData &coef_fourp)
    {
...
#pragma omp parallel for
    ...
    inline std::string get_proton_four_point_func_psel_path(const std::string &job_tag,
                                                            const int traj)
    {
        return ssprintf("analysis/proton_fourp_psel_coef/%s/results=%d", job_tag.c_str(),
                        traj);
    }
                                                         ...
                                                         const std::vector<int> &tsep_list)
    {
        ...
        for (int yt = 0; yt < total_site[3]; ++yt)
        {
            ...
            for (long ny = 0; ny < n_points; ++ny)
            {
                ...
                if ((xg_y[3] == 2) && (xg_y[0] == 17) && (xg_y[1] == 12) && (xg_y[2] == 6))
                {
                    continue;
                }
                Timer::autodisplay();
                ...
                for (int ti = 0; ti < tsep_num; ++ti)
                {
                    ...
                    contract_proton_pp_y_block(block, ppblock, psel_num_list, job_tag, traj, geo, psel,
                                               xg_y_psel_idx, xg_y, dtmin, tsep, coef_fourp);
                                                    ...
                                                    const int traj)
    {
        ...
        if (does_file_exist_sync_node(path + "/fourp-data-dt1-7.field"))
        {
            return;
        }
        ...
        check_time_limit();
        if (not obtain_lock(
                ssprintf("lock-proton-fourp-psel-coef-%s-%d", job_tag.c_str(), traj)))
        {
            return;
        }
        setup(job_tag, traj);
        qmkdir_info("analysis/proton_fourp_psel_coef");
        qmkdir_info(ssprintf("analysis/proton_fourp_psel_coef/%s", job_tag.c_str()));
        qmkdir_info(path);
