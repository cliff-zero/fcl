...
#pragma omp parallel for
    ...
    inline void contract_proton_fourp_unshifted_psel_acc_x_typeI(
        std::vector<Vector<Complex>> vv, const Complex coef,
        const WilsonMatrix &wm_snk_y, const ProtonPPBlock &block)
    {
        ...
        {
            const WilsonMatrix wm_pp_y_tsrc_tsnk_y = block.block_pp[gram] * wm_snk_y;
            for (int mu = 0; mu < 8; mu++)
            {
                vv[gram][mu] += coef * matrix_trace(wm_pp_y_tsrc_tsnk_y, va_ms[mu]);
            }
        }
        return;
    }
                                                           ...
                                                           const std::vector<std::vector<ProtonPPBlock>> &ppblock)
    {
        ...
        for (int dt = 0; dt < num_dtxy; ++dt)
        {
            ...
            for (long nsnk = 0; nsnk < n_points; ++nsnk)
            {
                ...
                const PselProp &psprop_y = get_psel_prop_psrc(job_tag, traj, xg_y, 0, 0);
                const WilsonMatrix &wm_snk_y = psprop_y.get_elem(xg_snk_psel_idx);
...
#pragma omp parallel for
                for (long idx = 0; idx < fsel.n_elems; ++idx)
                {
                    ...
                    if (tsrc <= tsnk)
                    {
                        contract_proton_fourp_unshifted_psel_acc_x_typeI(vv, coef, wm_snk_y, block1);
                    }
                    else
                    {
                        const Complex coef1 = -1.0 * coef;
                        contract_proton_fourp_unshifted_psel_acc_x_typeI(vv, coef1, wm_snk_y, block1);
                    }
                }
            }
        }
