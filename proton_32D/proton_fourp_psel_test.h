...
#pragma omp parallel for
    ...
    inline void contract_proton_pp_y_block(const std::vector<std::vector<std::vector<ProtonPPBlock>>> &block,
                                           ...
                                           const int dtmin, const int tsep)
    {
        TIMER_VERBOSE("contract_proton_pp_y_block");
        const Coordinate &total_site = geo.total_site();
        const int num_dtxy = 5;
...
#pragma omp parallel for
                                                            ...
                                                            const std::vector<std::vector<ProtonSPBlock>> &spblock)
    {
        ...
        for (int dt = 0; dt < num_dtxy; ++dt)
        {
            ...
            const int dt_y = mod(yt - tsrc, total_site[3]);
            //const Complex coef = 1.0 / (double)psel_dt_num_list[i_dt][dt_y] * (double)x_sparsity;
            const Complex coef = (double)x_sparsity;
...
#pragma omp parallel for
    ...
    // ssp = make_shift_shuffle_plan(fsel, -xg_y);
    {
        ...
        contract_proton_fourp_unshifted_psel_typeI(sfs, job_tag, traj, prop_x_y, xg_y, xg_y_psel_idx,
                                                   tsep, psel, fsel, psel_dt_num_list, dtmin, ppblock);
        qassert(sfs.size() == 5);
    ...
    // ssp = make_shift_shuffle_plan(fsel, -xg_y);
    {
        ...
        qassert(ssp.is_reflect == false);
        std::vector<SelectedField<Complex>> sfs;
        contract_proton_fourp_unshifted_psel_typeII(sfs, job_tag, traj, xg_y, xg_y_psel_idx, tsep, psel,
    ...
    inline std::string get_proton_four_point_func_psel_path(const std::string &job_tag,
                                                            const int traj)
    {
        return ssprintf("analysis/proton_fourp_psel_test/%s/results=%d", job_tag.c_str(),
                        traj);
    }
                                                    ...
                                                    const int traj)
    {
        ...
        if (does_file_exist_sync_node(path + "/fourp-data-dt2-7.field"))
        {
            return;
        }
        ...
        check_time_limit();
        if (not obtain_lock(
                ssprintf("lock-proton-fourp-psel-test-%s-%d", job_tag.c_str(), traj)))
        {
            return;
        }
        setup(job_tag, traj);
        qmkdir_info("analysis/proton_fourp_psel_test");
        qmkdir_info(ssprintf("analysis/proton_fourp_psel_test/%s", job_tag.c_str()));
        qmkdir_info(path);
