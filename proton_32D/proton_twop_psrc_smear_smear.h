...
{ //
    ...
    inline std::string get_proton_two_point_psrc_path(const std::string &job_tag,
                                                      const int traj)
    {
        return ssprintf("analysis/proton_twop_smear_smear_fsel/%s/results=%d", job_tag.c_str(),
                        traj);
    }
                                              ...
                                              const int traj)
    {
        ...
        for (long n = 0; n < n_points; n++)
        {
            ...
            iter += 1;
            const SelProp &pprop = get_prop_smear(job_tag, traj, xg_src, 0, 0, 1);
    ...
    inline void compute_proton_two_point_psrc_func(const std::string &job_tag, const int traj)
    {
        ...
        check_time_limit();
        if (not obtain_lock(
                ssprintf("lock-two-point-smear-smear-fsel-func-%s-%d", job_tag.c_str(), traj)))
        {
            return;
        }
        setup(job_tag, traj);
        qmkdir_info("analysis/proton_twop_smear_smear_fsel");
        qmkdir_info(ssprintf("analysis/proton_twop_smear_smear_fsel/%s", job_tag.c_str()));
        qmkdir_info(path);
