...
#pragma omp parallel for
                                                              ...
                                                              const std::vector<int> &tsep_list_snk)
    {
        ...
        for (int ti = 0; ti < tsep_num; ++ti)
        {
            ...
            fn_gram_list[ti] = path + ssprintf("/fourp-data-dt%d-%d-disconnected.field", tsep_snk, tsep_src);
        }
        ...
        for (long n = 0; n < n_points; ++n)
        {
            ...
            for (int ti = 0; ti < tsep_num; ++ti)
            {
                ...
                contract_proton_four_point_disc_acc_psel_coef(fourp_data[ti], job_tag, traj, prop_x_y, xg_y, xg_y_psel_idx,
                                                              tsep_src, tsep_snk, psel, fsel, ssp, psel_num_list, psel_dt_num_list,
                                                                 ...
                                                                 const int traj)
    {
        ...
        if (does_file_exist_sync_node(path + "/fourp-data-dt2-3-disconnected.field"))
        {
            return;
        }
