...
#pragma omp for
    ...
    inline std::string get_pion_two_point_smear_path(const std::string &job_tag,
                                                     const int traj)
    {
        return ssprintf("analysis/pion_twop_psel/%s/results=%d", job_tag.c_str(),
                        traj);
    }
                                             ...
                                             const int traj)
    {
        ...
        for (long n = 0; n < n_points; n++)
        {
            ...
            LatData ld = contract_pion_two_point_psrc_pselsnk_function(sp_prop, geo, tslice, psel);
    ...
    inline void compute_pion_two_point_smear_func(const std::string &job_tag, const int traj)
    {
        ...
        setup(job_tag, traj);
        qmkdir_info("analysis/pion_twop_psela");
        qmkdir_info(ssprintf("analysis/pion_twop_psela/%s", job_tag.c_str()));
        qmkdir_info(path);
