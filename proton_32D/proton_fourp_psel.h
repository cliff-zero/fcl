...
#pragma omp parallel for
    ...
    inline std::string get_proton_four_point_func_psel_path(const std::string &job_tag,
                                                            const int traj)
    {
        return ssprintf("analysis/proton_fourp_psel/%s/results=%d", job_tag.c_str(),
                        traj);
    }
                                                    ...
                                                    const int traj)
    {
        ...
        check_time_limit();
        if (not obtain_lock(
                ssprintf("lock-proton-fourp-psel-%s-%d", job_tag.c_str(), traj)))
        {
            return;
        }
        setup(job_tag, traj);
        qmkdir_info("analysis/proton_fourp_psel");
        qmkdir_info(ssprintf("analysis/proton_fourp_psel/%s", job_tag.c_str()));
        qmkdir_info(path);
