...
#pragma omp parallel for
    ...
    inline std::string get_proton_four_point_func_smear_path(const std::string &job_tag,
                                                             const int traj)
    {
        return ssprintf("analysis/proton_fourp_smear_9to11/%s/results=%d", job_tag.c_str(),
                        traj);
    }
                                                     ...
                                                     const int traj)
    {
        ...
        if (does_file_exist_sync_node(path + "/fourp-data-dt3-3-7.field"))
        {
            return;
        }
        ...
        setup(job_tag, traj);
        qmkdir_info("analysis/proton_fourp_smear_9to11");
        qmkdir_info(ssprintf("analysis/proton_fourp_smear_9to11/%s", job_tag.c_str()));
        qmkdir_info(path);
