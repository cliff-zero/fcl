...
{ //
    ...
    inline std::string get_proton_two_point_psrc_path(const std::string &job_tag,
                                                      const int traj)
    {
        return ssprintf("analysis/proton_twop/%s/results=%d", job_tag.c_str(),
                        traj);
    }
    ...
    inline void compute_proton_two_point_psrc_func(const std::string &job_tag, const int traj)
    {
        ...
        setup(job_tag, traj);
        qmkdir_info("analysis/proton_twop");
        qmkdir_info(ssprintf("analysis/proton_twop/%s", job_tag.c_str()));
        qmkdir_info(path);
