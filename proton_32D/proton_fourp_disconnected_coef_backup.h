...
#pragma omp parallel for
                                                                     ...
                                                                     const ProtonTwopBlock &block)
    {
        ...
        const WilsonMatrix wm_pp_y_tsrc_tsnk_x = block.c_twop * wm_y_x;
    ...
    inline void contract_proton_fourp_disc_unshifted_psel_coef(
        ...
        const std::vector<std::vector<long>> psel_dt_num_list,
        const int dtmin,
        ...
        const LatData &coef_table)
    {
        ...
        for (int dt = 0; dt < num_dtxy; ++dt)
        {
            ...
            const Complex coef_psel = 1.0 / (double)psel_dt_num_list[i_dt][dt_y] * (double)x_sparsity;
            ...
            for (long nsnk = 0; nsnk < n_points; ++nsnk)
            {
                ...
                for (long nsrc = 0; nsrc < n_points; ++nsrc)
                {
                    ...
                    const int dist1 = ceil(sqrt(sum(xg_sep1 * xg_sep1)) / dx_max * 20.0);
                    const int dist2 = ceil(sqrt(sum(xg_sep2 * xg_sep2)) / dx_max * 20.0);
                    const int dist3 = ceil(sqrt(sum(xg_sep3 * xg_sep3)) / dx_max * 20.0);
...
#pragma omp parallel for
                    for (long idx = 0; idx < fsel.n_elems; ++idx)
                    {
                        ...
                        if (tsrc <= tsnk)
                        {
                            contract_proton_fourp_disc_unshifted_psel_acc_x_coef(vv, coef, wm_x_y, tpblock);
                        }
                        else
                        {
                            const Complex coef1 = -1.0 * coef;
                            contract_proton_fourp_disc_unshifted_psel_acc_x_coef(vv, coef1, wm_x_y, tpblock);
                        }
                    }
    ...
    // ssp = make_shift_shuffle_plan(fsel, -xg_y);
    {
        ...
        contract_proton_fourp_disc_unshifted_psel_coef(sfs, job_tag, traj, prop_x_y, xg_y, xg_y_psel_idx,
                                                       tsep, psel, fsel, psel_num_list, psel_dt_num_list,
                                                       dtmin, block, coef_table);
    ...
    inline std::string get_proton_four_point_disc_func_noama_path_coef(const std::string &job_tag,
                                                                       const int traj)
    {
        return ssprintf("analysis/proton_fourp_disconnected_coef/%s/results=%d", job_tag.c_str(),
                        traj);
    }
                                                              ...
                                                              const std::vector<int> &tsep_list)
    {
        ...
        contract_proton_twop_block(block, psel_num_list, psel_dt_num_list, job_tag, traj, geo, psel, dtmax, dtmin);

        LatData coef_fourp;
        const std::string path_coef_table = ssprintf("coef-list/fourp/coef-fourp-20slice.lat");
        ...
        for (long n = 0; n < n_points; ++n)
        {
            ...
            for (int ti = 0; ti < tsep_num; ++ti)
            {
                ...
                contract_proton_four_point_disc_acc_psel_coef(fourp_data[ti], job_tag, traj, prop_x_y, xg_y, xg_y_psel_idx,
                                                              tsep, psel, fsel, ssp, psel_num_list, psel_dt_num_list,
                                                              dtmin, block, coef_fourp);
            }
        }
                                                                 ...
                                                                 const int traj)
    {
        ...
        check_time_limit();
        if (not obtain_lock(
                ssprintf("lock-proton-fourp-disc-coef-%s-%d", job_tag.c_str(), traj)))
        {
            return;
        }
        setup(job_tag, traj);
        qmkdir_info("analysis/proton_fourp_disconnected_coef");
        qmkdir_info(ssprintf("analysis/proton_fourp_disconnected_coef/%s", job_tag.c_str()));
        qmkdir_info(path);
        std::vector<int> tsep_list;
        tsep_list.push_back(2);
        compute_proton_four_point_disc_func_type_coef(job_tag, traj, tsep_list);
        release_lock();
    }
}
