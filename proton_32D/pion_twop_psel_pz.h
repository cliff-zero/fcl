...
{ //
    ...
    inline LatData mk_pion_two_point_sum_table(const Coordinate &total_site)
    {
        LatData ld;
    ...
    inline LatData contract_pion_psel_correction(const LatData &ld0, const std::vector<int> &psel_p_tslice,
                                                 const Geometry &geo)
    {
        const Coordinate total_site = geo.total_site();
        LatData ld = mk_pion_two_point_sum_table(total_site);
        set_zero(ld);
    ...
    inline LatData contract_pion_two_point_psrc_pselsnk_function(const PselProp &prop1, const Geometry &geo,
                                                                 const int tslice,
                                                                 const PointSelection &psel, const Coordinate xg_src)
    // m_ts[tsep][op_src][op_snk] = trace( (\sum_x prop1(x) gms[op_src] gamma5
    // prop2(x)^\dagger gamma5) gms[op_snk] ) 0 <= tsep < total_site[3]
    {
        TIMER_VERBOSE("contract_pion_two_point_psrc_pselsnk_function");
        const Coordinate total_site = geo.total_site();
...
#pragma omp for
            for (long idx = 0; idx < (long)psel.size(); ++idx)
            {
                ...
                const int xsep = mod(xg[0] - xg_src[0], total_site[0]);
                const Complex cp = cos(double(xsep) / double(total_site[0]) * 2.0 * 3.14159) + ii * sin(double(xsep) / double(total_site[0]) * 2.0 * 3.14159);
                c_ts[tsep] += pion_twop_block(wm) * cp;
            }
    ...
    inline std::string get_pion_two_point_smear_path(const std::string &job_tag,
                                                     const int traj)
    {
        return ssprintf("analysis/pion_twop_psel_pz/%s/results=%d", job_tag.c_str(),
                        traj);
    }

    inline void compute_pion_two_point_smear(const std::string &job_tag,
                                             const int traj)
    {
        check_sigterm();
        Timer::autodisplay();
        TIMER_VERBOSE("compute_pion_two_point_smear");
        const PointSelection &psel = get_point_selection(job_tag, traj);
        const long n_points = psel.size();
        const FieldSelection &fsel = get_field_selection(job_tag, traj);
        LatData ld_two_point_psrc_func;
        const std::string path = get_pion_two_point_smear_path(job_tag, traj);
        ...
        for (long n = 0; n < n_points; n++)
        {
            ...
            const PselProp &sp_prop = get_psel_prop_psrc(job_tag, traj, xg_src, 0, 0);
            LatData ld = contract_pion_two_point_psrc_pselsnk_function(sp_prop, geo, tslice, psel, xg_src);
            ld_two_point_psrc_func += ld;
        }
        lat_data_save_info(path_two_point_psrc, ld_two_point_psrc_func);
        LatData ld_two_point_psrc_func_sum =
            contract_pion_psel_correction(ld_two_point_psrc_func, psel_p_tslice, geo);
    ...
    inline void compute_pion_two_point_smear_func(const std::string &job_tag, const int traj)
    {
        check_sigterm();
        Timer::autodisplay();
        TIMER_VERBOSE("compute_pion_two_point_smear_func");
        const std::string path = get_pion_two_point_smear_path(job_tag, traj);
        ...
        setup(job_tag, traj);
        qmkdir_info("analysis/pion_twop_psel_pz");
        qmkdir_info(ssprintf("analysis/pion_twop_psel_pz/%s", job_tag.c_str()));
        qmkdir_info(path);
        compute_pion_two_point_smear(job_tag, traj);
        release_lock();
    }
