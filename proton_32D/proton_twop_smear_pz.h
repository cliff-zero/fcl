...
#pragma omp for
    ...
    inline std::string get_proton_two_point_smear_path(const std::string &job_tag,
                                                       const int traj)
    {
        return ssprintf("analysis/proton_twop_smear_pz/%s/results=%d", job_tag.c_str(),
                        traj);
    }
                                               ...
                                               const int traj)
    {
        ...
        for (long n = 0; n < n_points; n++)
        {
            ...
            LatData ld = contract_proton_two_point_psrc_pselsnk_function(sp_prop, geo, tslice, psel, xg_src);
    ...
    inline void compute_proton_two_point_smear_func(const std::string &job_tag, const int traj)
    {
        ...
        setup(job_tag, traj);
        qmkdir_info("analysis/proton_twop_smear_pz");
        qmkdir_info(ssprintf("analysis/proton_twop_smear_pz/%s", job_tag.c_str()));
        qmkdir_info(path);
