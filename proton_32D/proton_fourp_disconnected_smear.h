...
#pragma omp parallel for
    ...
    inline std::string get_proton_four_point_disc_func_noama_path_coef(const std::string &job_tag,
                                                                       const int traj)
    {
        return ssprintf("analysis/proton_fourp_disconnected_smear/%s/results=%d", job_tag.c_str(),
                        traj);
    }
	...
	*/
                                                                 ...
                                                                 const int traj)
    {
        ...
        if (does_file_exist_sync_node(path + "/fourp-data-dt2-2-disconnected.field"))
        {
            return;
        }
        ...
        check_time_limit();
        if (not obtain_lock(
                ssprintf("lock-proton-fourp-disc5-smear-%s-%d", job_tag.c_str(), traj)))
        {
            return;
        }
        setup(job_tag, traj);
        qmkdir_info("analysis/proton_fourp_disconnected_smear");
        qmkdir_info(ssprintf("analysis/proton_fourp_disconnected_smear/%s", job_tag.c_str()));
        qmkdir_info(path);
        ...
        //tsep_list_src.push_back(5); // small to large
        //tsep_list_snk.push_back(5);
