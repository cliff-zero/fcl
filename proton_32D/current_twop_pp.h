...
#pragma omp for
    ...
    inline std::string get_pion_two_point_psrc_path(const std::string &job_tag,
                                                    const int traj)
    {
        return ssprintf("analysis/current_twop_pp/%s/results=%d", job_tag.c_str(),
                        traj);
    }
    ...
    inline void compute_pion_two_point_psrc_func(const std::string &job_tag, const int traj)
    {
        ...
        setup(job_tag, traj);
        qmkdir_info("analysis/current_twop_pp");
        qmkdir_info(ssprintf("analysis/current_twop_pp/%s", job_tag.c_str()));
        qmkdir_info(path);
