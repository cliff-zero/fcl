...
{ //
    ...
    inline LatData contract_proton_two_point_psrc_pselsnk_function_gram1(const PselProp &prop1, const Geometry &geo,
                                                                         const int tslice,
...
#pragma omp for
    ...
    inline LatData contract_proton_two_point_psrc_pselsnk_function_gram2(const PselProp &prop1, const Geometry &geo,
                                                                         const int tslice,
                                                                         const PointSelection &psel)
    // m_ts[tsep][op_src][op_snk] = trace( (\sum_x prop1(x) gms[op_src] gamma5
    // prop2(x)^\dagger gamma5) gms[op_snk] ) 0 <= tsep < total_site[3]
...
#pragma omp for
    ...
    inline std::string get_proton_two_point_psrc_pselsnk_path(const std::string &job_tag,
                                                              const int traj)
    {
        return ssprintf("analysis/proton_twop_pselsnk_test/%s/results=%d", job_tag.c_str(),
                        traj);
    }
                                                      ...
                                                      const int traj)
    {
        ...
        LatData ld_two_point_psrc_func_gram1, ld_two_point_psrc_func_gram2;
        const std::string path = get_proton_two_point_psrc_pselsnk_path(job_tag, traj);
        const std::string path_two_point_psrc1 =
            path + ssprintf("/two-point-psrc-pselsnk-gram1.lat");
        const std::string path_two_point_psrc2 =
            path + ssprintf("/two-point-psrc-pselsnk-gram2.lat");
        ...
        for (long n = 0; n < n_points; n++)
        {
            ...
            LatData ld1 = contract_proton_two_point_psrc_pselsnk_function_gram1(pprop, geo, tslice, psel);
            LatData ld2 = contract_proton_two_point_psrc_pselsnk_function_gram2(pprop, geo, tslice, psel);
            ld_two_point_psrc_func_gram1 += ld1;
            ld_two_point_psrc_func_gram2 += ld2;
        }
        lat_data_save_info(path_two_point_psrc1, ld_two_point_psrc_func_gram1);
        lat_data_save_info(path_two_point_psrc2, ld_two_point_psrc_func_gram2);
        LatData ld_two_point_psrc_func_sum =
            contract_proton_psel_correction(ld_two_point_psrc_func_gram1, psel_p_tslice, geo);
    ...
    inline void compute_proton_two_point_psrc_pselsnk_func(const std::string &job_tag, const int traj)
    {
        check_sigterm();
        Timer::autodisplay();
        TIMER_VERBOSE("compute_proton_two_point_psrc_pselsnk_func_test");
        const std::string path = get_proton_two_point_psrc_pselsnk_path(job_tag, traj);
        ...
        if (does_file_exist_sync_node(
                path + "/two-point-psrc-pselsnk-gram2.lat"))
        {
            return;
        }
        ...
        check_time_limit();
        if (not obtain_lock(
                ssprintf("lock-two-point-pselsnk-func-test-%s-%d", job_tag.c_str(), traj)))
        {
            return;
        }
        setup(job_tag, traj);
        qmkdir_info("analysis/proton_twop_pselsnk_test");
        qmkdir_info(ssprintf("analysis/proton_twop_pselsnk_test/%s", job_tag.c_str()));
        qmkdir_info(path);
