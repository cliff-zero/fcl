...
{ //
    ...
    inline LatData contract_proton_psel_correction(const LatData &ld0, const std::vector<int> &psel_p_tslice,
                                                   const std::vector<int> &psel_p_tslice_snk, const Geometry &geo)
    {
        ...
        for (int tsep = 0; tsep < total_site[3]; ++tsep)
        {
            ...
            for (int tslice = 0; tslice < total_site[3]; ++tslice)
            {
                ...
                pselsum += psel_p_tslice[tslice] * psel_p_tslice_snk[tsnk];
...
#pragma omp for
    ...
    inline std::string get_proton_two_point_smear_path(const std::string &job_tag,
                                                       const int traj)
    {
        return ssprintf("analysis/proton_twop_smear_point/%s/results=%d", job_tag.c_str(),
                        traj);
    }
                                               ...
                                               const int traj)
    {
        ...
        const PointSelection &psel_snk = get_point_selection(job_tag, traj);
        ...
        std::vector<int> psel_p_tslice_snk(total_site[3]);
        // std::vector<int> pselpertsep(total_site[3]);
        set_zero(psel_p_tslice);
        set_zero(psel_p_tslice_snk);
        ...
        for (long n = 0; n < n_points; n++)
        {
            ...
            const PselProp &sp_prop = get_psel_prop_smear(job_tag, traj, xg_src, 0, 0, 0);
            LatData ld = contract_proton_two_point_psrc_pselsnk_function(sp_prop, geo, tslice, psel_snk);
            ld_two_point_psrc_func += ld;
            const long xg_temp_psel_idx = n;
            const Coordinate &xg_temp = psel[xg_temp_psel_idx];
            const int &ttemp = xg_temp[3];
            psel_p_tslice_snk[ttemp] += 1;
        }
        lat_data_save_info(path_two_point_psrc, ld_two_point_psrc_func);
        LatData ld_two_point_psrc_func_sum =
            contract_proton_psel_correction(ld_two_point_psrc_func, psel_p_tslice, psel_p_tslice_snk, geo);
    ...
    inline void compute_proton_two_point_smear_func(const std::string &job_tag, const int traj)
    {
        ...
        check_time_limit();
        if (not obtain_lock(
                ssprintf("lock-two-point-smear-point-func-%s-%d", job_tag.c_str(), traj)))
        {
            return;
        }
        setup(job_tag, traj);
        qmkdir_info("analysis/proton_twop_smear_point");
        qmkdir_info(ssprintf("analysis/proton_twop_smear_point/%s", job_tag.c_str()));
        qmkdir_info(path);
