...
namespace qlat
{
    inline std::string get_va_two_point_func_noama_path(const std::string &job_tag,
                                                        const int traj)
    {
        return ssprintf("analysis/va_twop_noama/%s/results=%d", job_tag.c_str(),
                        traj);
    }

    inline void compute_va_two_point_func_type_noama(const std::string &job_tag, const int traj)
    {
        check_sigterm();
        check_time_limit();
        Timer::autodisplay();
        const std::string path = get_va_two_point_func_noama_path(job_tag, traj);
        ...
        TIMER_VERBOSE("compute_va_two_point_func_type_noama");
        ...
        for (long n = 0; n < n_points; ++n)
        {
            ...
            Timer::autodisplay();
            TIMER_VERBOSE("compute_va_two_point_func_type_noama-iter");
            iter += 1;
            const ShiftShufflePlan ssp = make_shift_shuffle_plan(fsel, -xg_y);

            displayln_info(fname + ssprintf(":n=%ld iter=%ld", n,
                                            iter));
            contract_va_two_point_acc_noama(data_va_twop, job_tag, traj, xg_y, xg_y_psel_idx, psel,
                                            fsel, ssp);
        }
    ...
    inline void compute_va_two_point_func_noama(const std::string &job_tag,
                                                const int traj)
    {
        check_sigterm();
        Timer::autodisplay();
        TIMER_VERBOSE("compute_va_two_point_func_noama");
        const std::string path = get_va_two_point_func_noama_path(job_tag, traj);
        ...
        check_time_limit();
        if (not obtain_lock(
                ssprintf("lock-va-twop-noama-%s-%d", job_tag.c_str(), traj)))
        {
            return;
        }
        setup(job_tag, traj);
        qmkdir_info("analysis/va_twop_noama");
        qmkdir_info(ssprintf("analysis/va_twop_noama/%s", job_tag.c_str()));
        qmkdir_info(path);
        compute_va_two_point_func_type_noama(job_tag, traj);
        release_lock();
    }
}