...
#define PROTONPPBLOCK
    ...
    inline void contract_proton_pp_block(std::vector<std::vector<std::vector<ProtonPPBlock>>> &block,
                                         ...
                                         const Geometry &geo, const PointSelection &psel_smear, const PointSelection &psel,
...
#pragma omp parallel for
    ...
    inline std::string get_proton_three_point_func_smear_path(const std::string &job_tag,
                                                              const int traj)
    {
        return ssprintf("analysis/proton_threep_smear_point/%s/results=%d", job_tag.c_str(),
                        traj);
    }
                                                           ...
                                                           const std::vector<int> &gram_convert_list)
    {
        ...
        contract_proton_pp_block(block, psel_num_list, psel_dt_num_list, job_tag, traj, geo, psel_smear, psel, dtmax, dtmin);
                                                      ...
                                                      const int traj)
    {
        ...
        setup(job_tag, traj);
        qmkdir_info("analysis/proton_threep_smear_point");
        qmkdir_info(ssprintf("analysis/proton_threep_smear_point/%s", job_tag.c_str()));
        qmkdir_info(path);
