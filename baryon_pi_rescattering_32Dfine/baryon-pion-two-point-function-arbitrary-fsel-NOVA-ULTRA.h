...
namespace qlat
{
    ...
    inline std::string get_baryon_pi_rescattering_all_psel_arbitrarty_func_psel_path(const std::string &job_tag,
                                                                                     const int &traj)
    {
        return ssprintf("analysis/baryon_pi_rescattering_fsel_nova_ultra/%s/results=%d", job_tag.c_str(),
                        traj);
    }
    ...
    inline void contract_leading_tiwst_topology_fsel_ultra(
        ...
        const int &gram_num)
    {
        TIMER_VERBOSE("contract_leading_tiwst_topology_fsel_ultra");
...
#pragma omp parallel for collapse(3)
        ...
        const double &anti_period, const int &gram_num)
    {
        ...
        contract_leading_tiwst_topology_fsel_ultra(
            job_tag, traj,
...
#pragma omp parallel for collapse(2)
                                                                         ...
                                                                         const int &traj, const bool is_baryon_smear, const bool is_meson_smear)
    {
        ...
        setup(job_tag, traj);
        qmkdir_info("analysis/baryon_pi_rescattering_fsel_nova_ultra");
        qmkdir_info(ssprintf("analysis/baryon_pi_rescattering_fsel_nova_ultra/%s", job_tag.c_str()));
        qmkdir_info(path);
