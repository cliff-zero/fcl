...
{ //
  ...
  inline bool get_does_file_exist_qar(const std::string &fn)
  {
    ...
    if (not cache.has(key))
    {
      ...
      cache[key] = does_file_exist_sync_node(fn + ".qar");
    }
    return cache[key];
  }
  ...
  typedef Cache<std::string, PointsSelection> PointSelectionCache;
  ...
  {
    static PointSelectionCache cache("PointSelectionCache", 64, 2);
    return cache;
  }

  inline FieldSelectionCache &get_field_selection_cache()
  {
    static FieldSelectionCache cache("FieldSelectionCache", 64, 2);
    return cache;
  }

  inline ShuffledBitSetCache &get_shuffled_bit_set_cache()
  {
    static ShuffledBitSetCache cache("ShuffledBitSetCache", 64, 2);
    return cache;
  }

  inline PointSrcInfoCache &get_point_src_info_cache()
  {
    static PointSrcInfoCache cache("PointSrcInfoCache", 64, 2);
    return cache;
  }
  ...
  {
    static GaugeTransformCache cache("GaugeTransformCache", 64, 2);
    return cache;
  }

  inline PselPropCache &get_psel_prop_cache()
  {
    static PselPropCache cache("PselPropCache", 4096, 4);
    return cache;
  }

  inline PselPropCache &get_psel_prop_smear_cache()
  {
    static PselPropCache cache("PselPropSmearCache", 4096, 4);
    return cache;
  }

  inline SelPropCache &get_prop_psrc_cache()
  {
    static SelPropCache cache("PropPsrcCache", 4096, 2);
    return cache;
  }

  inline SelPropCache &get_prop_smear_cache()
  {
    static SelPropCache cache("PropSmearCache", 4096, 2);
    return cache;
  }
  ...
  inline const PointsSelection &get_point_selection_smear(const std::string &job_tag,
                                                         const int traj)
  {
    const std::string key = ssprintf("%s,%d,psel_smear", job_tag.c_str(), traj);
    PointSelectionCache &cache = get_point_selection_cache();
    if (not cache.has(key))
    {
      TIMER_VERBOSE("get_point_selection_smear");
      const std::string fn_point_selection =
          get_point_selection_smear_path(job_tag, traj);
      qassert(get_does_file_exist(fn_point_selection));
      cache[key] = load_point_selection_info(fn_point_selection);
    }
    return cache[key];
                                                   ...
                                                   const int traj)
  {
    ...
    if (not cache.has(key))
    {
      ...
      read_field_selection(cache[key], fn_field_selection);
  ...
  inline const ShuffledBitSet &get_shuffled_bit_set_smear(const std::string &job_tag,
                                                          const int traj,
                                                          const std::string &path)
  {
    const std::string key =
    ...
    if (not cache.has(key))
    {
      TIMER_VERBOSE("get_shuffled_bit_set_smear");
      const FieldSelection &fsel = get_field_selection(job_tag, traj);
      const PointsSelection &psel = get_point_selection_smear(job_tag, traj);
      FieldSelection fselc;
      fselc.f_rank = fsel.f_rank;
      add_field_selection(fselc.f_rank, psel);
      update_field_selection(fselc);
                        ...
                        const std::string &fn)
  {
    ...
    displayln_info(ssprintf("%s", path_full.c_str()));
    if (does_file_exist_qar(path_full))
    {
      load_selected_points(ps_prop, path_full);
  ...
  inline std::string get_prop_smear_key(const std::string &job_tag, const int traj,
                                        const Coordinate &xg, const int type,
                                        const int accuracy, const int smeared)
  {
    return ssprintf("%s,%d,%d,%s,%d,%d,smear", show(xg).c_str(), type, accuracy,
                    job_tag.c_str(), traj, smeared);
  }
  ...
  inline const PselProp &get_psel_prop_smear(const std::string &job_tag,
                                             const int traj, const Coordinate &xg,
                                             const int type, const int accuracy, const int smeared)
  {
    const std::string key = get_prop_smear_key(job_tag, traj, xg, type, accuracy, smeared);
    PselPropCache &ps_cache = get_psel_prop_smear_cache();
    if (not ps_cache.has(key))
    {
      TIMER_VERBOSE("get_psel_prop_smear");
      PselProp &ps_prop = ps_cache[key];
      const std::string path_psel = get_psel_prop_smear_path(job_tag, traj, type);
      const std::string fn = get_smear_tag(xg, type, accuracy, smeared);
      load_prop(ps_prop, path_psel, fn);
    }
    return ps_cache[key];
  }
  ...
  inline const SelProp &get_prop_smear(const std::string &job_tag, const int traj,
                                       const Coordinate &xg, const int type,
                                       const int accuracy, const int smeared)
  {
    const std::string key = get_prop_smear_key(job_tag, traj, xg, type, accuracy, smeared);
    SelPropCache &s_cache = get_prop_smear_cache();
    if (not s_cache.has(key))
    {
      TIMER_VERBOSE("get_prop_smear");
      PselPropCache &ps_cache = get_psel_prop_smear_cache();
      PselProp &ps_prop = ps_cache[key];
      SelProp &s_prop = s_cache[key];
      const std::string path = get_prop_smear_path(job_tag, traj, type);
      const std::string fn = get_smear_tag(xg, type, accuracy, smeared);
      const PointsSelection &psel = get_point_selection(job_tag, traj);
      const FieldSelection &fsel = get_field_selection(job_tag, traj);
      const ShuffledBitSet &sbs = get_shuffled_bit_set(job_tag, traj, path);
      ...
      if (is_check_prop_consistency())
      {
        TIMER_VERBOSE("check_prop_consistency");
        const std::string path_psel =
            get_psel_prop_smear_path(job_tag, traj, type);
        PselProp ps_prop_load;
        load_prop(ps_prop_load, path_psel, fn);
        const double qnorm_ps_prop = qnorm(ps_prop_load);
        ps_prop_load -= ps_prop;
                              ...
                              const int type)
  {
    ...
    return get_does_file_exist(get_prop_psrc_path(job_tag, traj, type));
  }

  inline bool check_prop_smear(const std::string &job_tag, const int traj,
                               const int type)
  {
    TIMER_VERBOSE("check_prop_psrc");
    return get_does_file_exist_qar(get_prop_smear_path(job_tag, traj, type)) and
           get_does_file_exist_qar(get_psel_prop_smear_path(job_tag, traj, type));
  }
