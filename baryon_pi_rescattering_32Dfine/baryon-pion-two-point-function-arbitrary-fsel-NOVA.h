...
namespace qlat
{
    ...
    inline std::string get_baryon_pi_rescattering_all_psel_arbitrarty_func_psel_path(const std::string &job_tag,
                                                                                     const int &traj)
    {
        return ssprintf("analysis/baryon_pi_rescattering_fsel_nova/%s/results=%d", job_tag.c_str(),
                        traj);
    }

    inline LatData mk_baryon_pi_rescattering_fsel_for_arbitrary(const int &num_dtxy, const int &gram_num, const std::vector<std::vector<int>> &Momentum_Targets_src, const std::vector<std::vector<int>> &Momentum_Targets_snk, const Coordinate &total_site)
    {
        LatData ld;
    ...
    inline void contract_leading_tiwst_topology_fsel_nova(
        const std::string &job_tag, const int &traj,
        const std::vector<std::vector<std::vector<Leading_Twist_Block_No_Projection_Psel_Psel>>> &block,
        ...
        const int &gram_num)
    {
        TIMER_VERBOSE("contract_leading_tiwst_topology_fsel_nova");
...
#pragma omp parallel for collapse(3)
        for (int idx_baryon_src = 0; idx_baryon_src < psel_baryon_num_list[t_baryon_src]; idx_baryon_src++)
        {
            for (int idx_baryon_snk = 0; idx_baryon_snk < psel_baryon_num_list[t_baryon_snk]; idx_baryon_snk++)
            {
                for (int idx_meson_ty = 0; idx_meson_ty < psel_meson_num_list[t_meson]; idx_meson_ty++)
                {
                    ...
                    const Leading_Twist_Block_No_Projection_Psel_Psel &LT_block_np = block[idx_baryon_snk][idx_baryon_src][idx_meson_ty];

                    qassert(LT_block_np.is_build);
                    qassert(LT_block_np.xg_snk_psel_idx == xg_snk_psel_idx);
                    qassert(LT_block_np.xg_src_psel_idx == xg_src_psel_idx);
                    qassert(LT_block_np.xg_y_psel_idx == xg_y_psel_idx);
                    ...
                    for (int idx_x = 0; idx_x < fsel_num_list[xt]; idx_x++)
                    {
                        ...
                        for (int VA_idx = 0; VA_idx < 8; VA_idx++)
                        {
                            ...
                            contract_swm_ppblock_np_nova(spin_result_list_temp, spin_result_list_ex_temp, LT_block_np, block_x4snk_y_temp, block_x4y_src_temp);
    ...
    inline void leading_tiwst_fsel_nova(
        const std::string &job_tag, const int &traj,
        const std::vector<std::vector<std::vector<std::vector<std::vector<Complex>>>>> &baryon_pi_two_func_coeff,
        const std::vector<std::vector<int>> &Momentum_Targets_src, const std::vector<std::vector<int>> &Momentum_Targets_snk,
        const bool &is_baryon_smear,
    ...
    {
        TIMER_VERBOSE("leading_tiwst_fsel_nova");
        ...
        contract_leading_twist_pselpsel_no_projection_block(
            job_tag, traj,
            is_baryon_smear,
            psel_baryon,
            psel_baryon_num,
        ...
        contract_leading_tiwst_topology_fsel_nova(
            job_tag, traj,
            block, baryon_pi_two_func_coeff,
...
#pragma omp parallel for collapse(2)
    ...
    inline void highter_tiwst_fsel(
        ...
        const double &anti_period, const int &gram_num)
    {
        TIMER_VERBOSE("highter_tiwst_fsel");
        std::vector<std::vector<std::vector<std::vector<Sequential_Prop_1pt_Block>>>> block_x_fsel;
        std::vector<std::vector<std::vector<Sequential_Prop_1pt_Block>>> block_y;
...
#pragma omp parallel for collapse(2)
                                                                         ...
                                                                         const int &traj, const bool is_baryon_smear, const bool is_meson_smear)
    {
        ...
        setup(job_tag, traj);
        qmkdir_info("analysis/baryon_pi_rescattering_fsel_nova");
        qmkdir_info(ssprintf("analysis/baryon_pi_rescattering_fsel_nova/%s", job_tag.c_str()));
        qmkdir_info(path);
