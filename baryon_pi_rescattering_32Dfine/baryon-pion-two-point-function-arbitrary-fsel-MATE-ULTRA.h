...
namespace qlat
{
    ...
    inline std::string get_baryon_pi_rescattering_all_psel_arbitrarty_func_psel_path(const std::string &job_tag,
                                                                                     const int &traj)
    {
        return ssprintf("analysis/baryon_pi_rescattering_fsel_mate_ultra/%s/results=%d", job_tag.c_str(),
                        traj);
    }
...
#pragma omp parallel for collapse(3)
                                        ...
                                        for (int topology_id = 0; topology_id < 5; topology_id++)
                                        {
                                            ...
                                            for (int mu1 = 0; mu1 < 4; mu1++)
                                            {
                                                for (int mu2 = 0; mu2 < 4; mu2++)
                                                {
                                                    ...
                                                    leading_tiwst_result[omp_get_thread_num()][(((((mom_src_id * mom_num_curr + mom_curr_id) * mom_num_snk + mom_snk_id) * gram_num + topology_id) * 8 + VA_idx) * 4 + mu1) * 4 + mu2] += spin_result_list(mu1, mu2);
                                                }
                                            }
                                            ...
                                            for (int mu1 = 0; mu1 < 4; mu1++)
                                            {
                                                for (int mu2 = 0; mu2 < 4; mu2++)
                                                {
                                                    ...
                                                    leading_tiwst_result[omp_get_thread_num()][(((((mom_src_id * mom_num_curr + mom_curr_id) * mom_num_snk + mom_snk_id) * gram_num + topology_id + 5) * 8 + VA_idx) * 4 + mu1) * 4 + mu2] += spin_result_list_ex(mu1, mu2);
                                                }
                                            }
                                        }
                                    }
        ...
        const double &anti_period, const int &gram_num)
    {
        ...
        contract_sequential_block_x_fsel_y_psel_mate_ultra(
            job_tag, traj,
            block_coll,
            block_coll_ex,
            block_x_y,
            ...
            for (int idx_baryon_snk = 0; idx_baryon_snk < psel_baryon_num_list[t_baryon_snk]; idx_baryon_snk++)
            {
                ...
                for (int idx_meson_ty = 0; idx_meson_ty < psel_meson_num_list[t_meson]; idx_meson_ty++)
                {
                    ...
                    for (int idx_x = 0; idx_x < (0 == fsel_num_list[xt] ? 0 : 1); idx_x++)
                    {
                        ...
                        for (int VA_idx = 0; VA_idx < 8; VA_idx++)
                        {
                            const Sequential_Prop_1pt_Block &block_x_temp = block_x_fsel[idx_baryon_snk][idx_baryon_src][idx_x][VA_idx];
                                        ...
                                        for (int topology_id = 0; topology_id < gram_num; topology_id++)
                                        {
                                            ...
                                            for (int mu1 = 0; mu1 < 4; mu1++)
                                            {
                                                for (int mu2 = 0; mu2 < 4; mu2++)
                                                {
                                                    ...
                                                    highter_tiwst_result[omp_get_thread_num()][(((((mom_src_id * mom_num_curr + mom_curr_id) * mom_num_snk + mom_snk_id) * gram_num + topology_id) * 8 + VA_idx) * 4 + mu1) * 4 + mu2] += spin_result_list(mu1, mu2);
                                                }
                                            }
                                        }
                                    }
...
#pragma omp parallel for collapse(2)
        for (int mom_src_id = 0; mom_src_id < mom_num_src; ++mom_src_id)
        {
            for (int mom_curr_id = 0; mom_curr_id < mom_num_curr; ++mom_curr_id)
            {
                for (int mom_snk_id = 0; mom_snk_id < mom_num_snk; ++mom_snk_id)
                {
                    Vector<ComplexD> baryon_pi_data_vec_temp = lat_data_complex_get(baryon_pi_data_temp, make_array(dtxy, mom_src_id, mom_curr_id, mom_snk_id));
                        ...
                        for (int topology_id = 0; topology_id < gram_num; topology_id++)
                        {
                            for (int VA_idx = 0; VA_idx < 8; VA_idx++)
                            {
                                for (int mu1 = 0; mu1 < 4; mu1++)
                                {
                                    for (int mu2 = 0; mu2 < 4; mu2++)
                                    {
                                        baryon_pi_data_vec_temp[((topology_id * 8 + VA_idx) * 4 + mu1) * 4 + mu2] += leading_tiwst_result[thread_id][(((((mom_src_id * mom_num_curr + mom_curr_id) * mom_num_snk + mom_snk_id) * gram_num + topology_id) * 8 + VA_idx) * 4 + mu1) * 4 + mu2];
                                        baryon_pi_data_vec_temp[((((topology_id + gram_num) * 8 + VA_idx)) * 4 + mu1) * 4 + mu2] += highter_tiwst_result[thread_id][(((((mom_src_id * mom_num_curr + mom_curr_id) * mom_num_snk + mom_snk_id) * gram_num + topology_id) * 8 + VA_idx) * 4 + mu1) * 4 + mu2];
                                    }
                                }
                            }
                        }
        ...
        const bool is_baryon_smear, const bool is_meson_smear)
    {
        ...
        for (int ti = 0; ti < tsep_num; ++ti)
        {
            ...
            fn_gram_list[ti] = path + ssprintf("/baryon_%s_pi_%s_rescattering_mate_fsel_tsep_%d_%d.field", (is_baryon_smear ? "smear" : "point"), (is_meson_smear ? "smear" : "point"), tsep_src, tsep_snk);
        }
        ...
        for (int ti = 0; ti < tsep_num; ++ti)
        {
            baryon_pi_data_result[ti] = mk_baryon_pi_rescattering_fsel_for_arbitrary_mate(num_dtxy, gram_num, Momentum_Targets_src, Momentum_Targets_curr, Momentum_Targets_snk, total_site);
        }
        ...
        baryon_pion_two_point_arbitrary_coeffience_mate_fsel(
            job_tag, traj,
            baryon_pi_two_func_coeff,
            psel_baryon_num_list,
            psel_meson_num_list,
        ...
        for (int ti = 0; ti < tsep_num; ++ti)
        {
            ...
            // for (int t_meson = 0; t_meson < 1; ++t_meson)
            {
                for (int dt = 0; dt < num_dtxy; ++dt)
                {
                    ...
                    for (int thread_id = 0; thread_id < omp_get_max_threads(); thread_id++)
                    {
                        leading_tiwst_result[thread_id].resize(mom_num_src * mom_num_curr * mom_num_snk * gram_num * 8 * 4 * 4);
                        highter_tiwst_result[thread_id].resize(mom_num_src * mom_num_curr * mom_num_snk * gram_num * 8 * 4 * 4);
                                                                         ...
                                                                         const int &traj, const bool is_baryon_smear, const bool is_meson_smear)
    {
        ...
        if (does_file_exist_sync_node(path + ssprintf("/baryon_%s_pi_%s_rescattering_mate_fsel_tsep_%d_%d.field", (is_baryon_smear ? "smear" : "point"), (is_meson_smear ? "smear" : "point"), 0, 0)))
        {
            return;
        }
        ...
        setup(job_tag, traj);
        qmkdir_info("analysis/baryon_pi_rescattering_fsel_mate_ultra");
        qmkdir_info(ssprintf("analysis/baryon_pi_rescattering_fsel_mate_ultra/%s", job_tag.c_str()));
        qmkdir_info(path);
        ...
        tsep_snk_list.push_back(1);
        tsep_snk_list.push_back(2);
        tsep_snk_list.push_back(3);
