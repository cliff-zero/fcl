...
#pragma omp for
    ...
    inline std::string get_proton_two_point_smear_path(const std::string &job_tag,
                                                       const int traj)
    {
        return ssprintf("analysis/proton_twop_smear_check/%s/results=%d", job_tag.c_str(),
                        traj);
    }
    ...
    inline void compute_proton_two_point_smear_func(const std::string &job_tag, const int traj)
    {
        ...
        setup(job_tag, traj);
        qmkdir_info("analysis/proton_twop_smear_check");
        qmkdir_info(ssprintf("analysis/proton_twop_smear_check/%s", job_tag.c_str()));
        qmkdir_info(path);
