...
namespace qlat
{
    ...
    inline std::string get_baryon_pi_rescattering_all_psel_arbitrarty_func_psel_path(const std::string &job_tag,
                                                                                     const int &traj)
    {
        return ssprintf("analysis/baryon_pi_rescattering_fsel_mate_ultra_magic/%s/results=%d", job_tag.c_str(),
                        traj);
    }
                            ...
                            for (int mom_src_id = 0; mom_src_id < mom_num_src; ++mom_src_id)
                            {
                                for (int mom_curr_id = 0; mom_curr_id < mom_num_curr; ++mom_curr_id)
                                {
                                    for (int mom_snk_id = 0; mom_snk_id < mom_num_snk; ++mom_snk_id)
                                    {
                                        ...
                                        const ComplexD normal_coeff = baryon_pi_two_func_coeff[ti][t_src_snk][mom_src_id][mom_snk_id][dtxy][phase] * (ComplexD)anti_period;
...
#pragma omp parallel for collapse(2)
        ...
        const bool is_baryon_smear, const bool is_meson_smear)
    {
        ...
        for (int ti = 0; ti < tsep_num; ++ti)
        {
            ...
            for (int t_src_snk = 0; t_src_snk < num_dtxy; ++t_src_snk)
            {
                baryon_pi_data_result[ti][t_src_snk] = mk_baryon_pi_rescattering_fsel_for_arbitrary_mate(t_src_snk, gram_num, Momentum_Targets_src, Momentum_Targets_curr, Momentum_Targets_snk, total_site);
            }
        }
        ...
        for (int ti = 0; ti < tsep_num; ++ti)
        {
            ...
            for (int t_src_snk = 0; t_src_snk < num_dtxy; ++t_src_snk)
            {
                ...
                // for (int t_baryon_src = 0; t_baryon_src < 1; ++t_baryon_src)
                {
                                                                         ...
                                                                         const int &traj, const bool is_baryon_smear, const bool is_meson_smear)
    {
        ...
        setup(job_tag, traj);
        qmkdir_info("analysis/baryon_pi_rescattering_fsel_mate_ultra_magic");
        qmkdir_info(ssprintf("analysis/baryon_pi_rescattering_fsel_mate_ultra_magic/%s", job_tag.c_str()));
        qmkdir_info(path);
