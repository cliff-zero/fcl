...
{ //
  ...
  void bcast(PointInfo &info, const int root = 0)
  {
    MPI_Bcast(&info, sizeof(PointInfo), MPI_BYTE, root, MPI_COMM_WORLD);
  }

  void bcast(std::vector<PointInfo> &recv, const int root = 0)
  {
    if (1 == get_num_node())
    {
      return;
    ...
    Long size = recv.size();
    bcast(size, root); // Broadcast the size of the vector

    // Resize the vector before broadcasting
    recv.resize(size);

    // Broadcast each PointInfo element in the vector
    for (auto &point : recv)
    {
      bcast(point, root);
    }
